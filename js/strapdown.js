...
+href
+'"'
+(href.substring(0,4) == 'http' && href.indexOf(location.hostname) == -1 ? ' target=_blank':'')
+'>'
+text
+'</a>';continue;}
...
;
PR.registerLangHandler(PR.createSimpleLexer([["pln",/^[\t\n\r \xa0]+/,null,"\t\n\r \u00a0"],["str",/^(?:"(?:[^"\\]|\\[\S\s])*(?:"|$)|'(?:[^'\\]|\\[\S\s])*(?:'|$))/,null,"\"'"]],[["com",/^--(?:\[(=*)\[[\S\s]*?(?:]\1]|$)|[^\n\r]*)/],["str",/^\[(=*)\[[\S\s]*?(?:]\1]|$)/],["kwd",/^(?:and|break|false|in|local|nil|not|or|repeat|return|true|print|require|loadfile|dofile|pcall|call)\b/,null],["tag",/^(?:while|for|do|end|if|else|elseif|then|until|function)\b/,null],["atn",/^(?:swop|dump|parse_www_form|next_post_field|read_post_field_chunk|read_request_body|newthread|echo|die|flush|header|clear_header|setcookie|session|coroutine_wait|wait|json_encode|json_decode|send|read|connect|close|tcp|udp|cosocket|get_post_body|is_dir|is_file|mkdir|rmdir|readdir|unlink|stat|date|explode|implode|hook|strip|trim|escape|escape_uri|unescape_uri|base64encode|base64decode|printf|sprintf|startsWith|endsWith|md5|iconv|date)\b/,null],["atv",/^(?:eio|_GET|_POST|headers)\b/,null],["lit",/^[+-]?(?:0x[\da-f]+|(?:\.\d+|\d+(?:\.\d*)?)(?:e[+-]?\d+)?)/i],
["pln",/^[_a-z]\w*/i],["pun",/^[^\w\t\n\r \xa0][^\w\t\n\r "'+=\xa0-]*/]]),["lua"]);
...
function insertAfter(referenceNode, newNode) {
  ...
  var els = document.getElementsByTagName('xmp');
for(var I=0;I<els.length;I++){var markdownEl = els[I];
  //////////////////////////////////////////////////////////////////////
  //
  // <head> stuff
  //
  ...
  newNode.className = 'markdown-container';
...
markdown = markdown.replace(/&#123;/g, '{');
  ...
  insertAfter(markdownEl, newNode);
  
  var m = document.getElementById('xmp-menu');
  console.log(m.innerHTML, newNode.innerHTML);
  if(m){
    var cs = newNode.childNodes;
    var odiv = null;
    for(var i=0;i<cs.length;i++){
        if(cs[i] && cs[i].tagName){
            if(cs[i].tagName.toUpperCase() == 'H2'){
                var n = document.createElement('li');
                n.innerHTML = '<a href="#'+cs[i].innerText+'">'+cs[i].innerText+'</a>';
                m.appendChild(n);
                n = document.createElement('div');
                m.appendChild(n);
                odiv = n;
                cs[i].innerHTML = '<a name="'+cs[i].innerText+'"></a>'+cs[i].innerHTML;
            }else if(cs[i].tagName.toUpperCase() == 'H3'){
                oldn = cs[i];
                var n = odiv;
                if(!n)n = document.createElement('div');
                n.innerHTML = n.innerHTML+'<ol class="'+cs[i].tagName.toUpperCase()+'"><li id="_'+cs[i].innerText+'"><a href="#'+cs[i].innerText+'">'+cs[i].innerText+'</a></li></ol>';
                m.appendChild(n);
                cs[i].innerHTML = '<a name="'+cs[i].innerText+'"></a>'+cs[i].innerHTML;
            }
        }
  ...
  for (var i=0, ii=codeEls.length; i<ii; i++) {
    ...
    if(!lang || lang.length < 1)lang = 'lua';
    codeEl.className = 'prettyprint linenums lang-' + lang;
  }
  prettyPrint();
